...
You can filter the types of answers that Bing includes in the response (for example images, videos, and news) by using the [responseFilter](reference/query-parameters.md#responsefilter) query parameter, which is a comma-delimited list of answers. An answer will be included in the response if Bing finds relevant content for it. 
...
```  
GET https://api.bing.microsoft.com/bing/v7.0/search?q=sailing+dinghies&responseFilter=images%2Cvideos%2Cnews&mkt=en-us HTTP/1.1  
...
Although Bing did not return video and news results in the previous response, it does not mean that video and news content does not exist. It simply means that the page didn't include them. However, if you [page](paging-results.md) through more results, the subsequent pages would likely include them. Also, if you call the [Video Search API](../bing-video-search/overview.md) and [News Search API](../bing-news-search/overview.md) endpoints directly, the response would likely contain results.

You are discouraged from using `responseFilter` to get results from a single API. If you want content from a single Bing API, call that API directly. For example, to receive only images, send a request to the Image Search API endpoint, `https://api.bing.microsoft.com/bing/v7.0/images/search` or one of the other [Images](../bing-image-search/reference/endpoints.md) endpoints. Calling the single API is important not only for performance reasons but because the content-specific APIs offer richer results. For example, you can use filters that are not available to the Web Search API to filter the results.  
...
```
https://api.bing.microsoft.com/bing/v7.0/search?q=sailing+dinghies+site:contososailing.com&mkt=en-us
```
...
To limit the web answer results to webpages that Bing discovered during a specific period, set the [freshness](reference/query-parameters.md#freshness) query parameter to one of the following case-insensitive values:
...
To limit the number of answers that Bing returns to the top two answers (webpages and images), set the [answerCount](reference/query-parameters.md#answercount) query parameter to 2.

```  
GET https://api.bing.microsoft.com/bing/v7.0/search?q=sailing+dinghies&answerCount=2&mkt=en-us HTTP/1.1  
Ocp-Apim-Subscription-Key: 123456789ABCDE  
User-Agent: Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 822)  
X-Search-ClientIP: 999.999.999.999  
X-Search-Location:  47.60357;long:-122.3295;re:100  
...
If the top ranked answers that Bing returns for a query are webpages, images, videos, and relatedSearches, the response would include those answers. If you set [answerCount](reference/query-parameters.md#answercount) to two (2), Bing returns the top two ranked answers: webpages and images. If you want Bing to include images and videos in the response, specify the [promote](reference/query-parameters.md#promote) query parameter and set it to images and videos.

```  
GET https://api.bing.microsoft.com/bing/v7.0/search?q=sailing+dinghies&answerCount=2&promote=images%2Cvideos&mkt=en-us HTTP/1.1  
Ocp-Apim-Subscription-Key: 123456789ABCDE  
User-Agent: Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 822)  
X-Search-ClientIP: 999.999.999.999  
X-Search-Location:  47.60357;long:-122.3295;re:100  
